...
const ContentImage = ({ item, idx, isMobile }) => (
  <div
    className={`relative ${
      isMobile
        ? "w-[80px] h-[80px] mb-2"
...
const ContentText = ({ item, idx, isMobile }) => (
  <motion.div
    initial={{
      opacity: 0,
...
const ContentCard = ({ item, idx, isMobile }) => (
  <motion.div
    key={idx}
    initial={{
      opacity: 0,
  ...
  >
    ...
    <ContentImage item={item} idx={idx} isMobile={isMobile} />
    <ContentText item={item} idx={idx} isMobile={isMobile} />
  </motion.div>
);
  ...
  <div className='relative flex flex-col items-center px-3 sm:px-4 md:px-6 lg:px-12 mb-10 sm:mb-0'>
    ...
    >
      {CONTENT_DATA.map((item, idx) => (
        <ContentCard key={idx} item={item} idx={idx} isMobile={isMobile} />
      ))}
    </div>
